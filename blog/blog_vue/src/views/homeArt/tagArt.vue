...
export default {
  ...
  methods: {
    async getData () {
      if (this.isBottom === false) {
        this.currentPage++
        const { data } = await axios.get(`/api/artData?currentPage=${this.currentPage}&currentStrip=${this.currentStrip}&inputValue=${this.inputValue}&sort=${this.sort}&selectTag=${this.selectTag}`)
  ...
  created () {
    this.selectTag = this.$route.params.value
